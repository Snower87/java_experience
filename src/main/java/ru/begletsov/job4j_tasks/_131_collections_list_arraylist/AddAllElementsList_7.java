...
/**
 * @Задание 7. Добавление списка в другой список
 * @Раздел 1.3.1. Коллекция List, ArrayList
 * @Описание Ответим на 2 вопроса:
 ...
 * - boolean addAll​(Collection<? extends E> c) - метод добавляет коллекцию с (в нашем случае - список) в список,
 * на котором мы вызываем этот метод. Стоит упомянуть что так можно добавить все типы коллекций, которые наследуются от интерфейса Collection.
 ...
 * ВАЖНО! Метод remove(E e) реализован с помощью цикла for(), подразумевает под собой первоначальный поиск удаляемого элемента
 ...
 * Задание: метод принимает 2 списка и элемент, который возможно содержится в этих списках. Вам необходимо проверить
 * оба списка, содержат ли они одновременно этот элемент, если содержит - необходимо удалить элемент из первого списка.
...
public class AddAllElementsList_7 {
