...
/**
 * @Задание 12. ListIterator для списка
 * @Раздел 1.3.1. Коллекция List, ArrayList
 * @Описание В интерфейсе List<E> определен особый метод listIterator(), который возвращает объект ListIterator<E>  и
 ...
 * - ListIterator < E >  listIterator (int index) - возвращает итератор списка для элементов в этом списке, начиная
 * с элемента индекс которого равен index.
 * Вы скажете - у нас уже есть Iterator<E>, для чего нам нужен еще один? Ответ прост - названия у них похожи, но
 ...
 * Задание: необходимо раскомментировать, заполнить пропуски (обозначены ...) так, чтобы мы сначала прошлись по списку
 * в прямом порядке, а потом в обратном.
 * @author Sergei Begletsov
 * @since 06.05.2021
 * @version 1
 */

public class ListIteratorUsage_12 {
