...
/**
 * @Задание 9. Удаление пар ключ-значение из отображения
 * @Раздел 1.3.3. Map. HashMap. LinkedHashMap
 * @Описание Мы рассмотрели как добавить ключ-значение в отображение, как значения изменять, объединять. Теперь осталось
 * рассмотреть способы удаления значений из отображения. Тут все просто, для этого в интерфейсе Map<K, V> определены следующие методы:
 ...
 * если переданный ключ key в настоящее время сопоставлен со значением value, которое также передано в методе. При этом
 * метод возвращает true если значение было удалено.
